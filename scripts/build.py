...
def page(lang):
    ...
    # The paper PDF is not hosted here: both links resolve to the arXiv record.
    paper = DATA['seo']['arxiv_pdf_url']
    paper_page = DATA['seo']['arxiv_url']
    arxiv_id = DATA['seo']['arxiv_id']
...
{narrative['growth']}
<section id="resources" class="section resources soft"><div class="container"><div class="resource-top"><div><p class="eyebrow">09 / {t('PAPER & RESOURCES','论文与资源')}</p><h2>{t('Read the full paper.','阅读全文。')}</h2><p>{t('The paper covers the formulation, the Skill Block contract, the LIBERO and LIBERO-Plus evaluations, the controlled studies, and the scaling hypothesis, with appendices on cost regimes and experimental details.','论文包含问题定义、Skill Block 契约、LIBERO 与 LIBERO-Plus 评测、受控研究以及缩放假设，并在附录中给出成本模型与实验细节。')}</p></div><a class="button primary" href="{paper_page}" target="_blank" rel="noopener">{t('Read the paper · arXiv','阅读论文 · arXiv')} ↗</a></div><div class="resource-links"><a href="{paper_page}" target="_blank" rel="noopener">arXiv:{arxiv_id} ↗</a><a href="https://github.com/IRMVLab/TGL" target="_blank" rel="noopener">{t("Method code · IRMVLab/TGL","方法代码 · IRMVLab/TGL")} ↗</a><a href="#demos">{t('10 demonstration videos','10 段演示视频')} ↗</a><a href="https://changnie.top" target="_blank" rel="noopener">{t("Chang Nie · Personal homepage","聂畅 · 个人主页")} ↗</a><a href="mailto:changniep@gmail.com">{t('Contact the authors','联系作者')} ↗</a></div><p class="small">{t('The method implementation is maintained in IRMVLab/TGL. Installation and execution instructions are in its README.','方法实现在 IRMVLab/TGL 仓库维护，安装与运行说明请见该仓库 README。')}</p><div class="citation" id="citation"><div><h3>BibTeX</h3><button id="copy-citation" type="button" hidden>{t('Copy citation','复制引用')}</button></div><pre><code id="bibtex">{e(bib)}</code></pre><span id="copy-status" role="status"></span></div></div></section>
{narrative['glossary']}
{narrative['faq']}
</main><footer class="container"><a class="brand" href="#">TGL<span class="brand-dot">.</span></a><p>Teach and Grow · Shanghai Jiao Tong University</p><a href="#">{t('Back to top','返回顶部')} ↑</a><p class="small research-bridge">{t('Research context: TGL studies agent-centered robot learning alongside emerging directions such as agent-as-policy systems, coding agents for robots, physical in-context learning, VLA and robot foundation models, and frontier-model robotic manipulation.','研究背景：TGL 研究以智能体为中心的机器人学习，与 agent-as-policy 系统、面向机器人的 coding agent、物理上下文学习、VLA 与机器人基础模型、前沿模型机器人操作等新兴方向并行。')}</p><nav class="footer-index footer-pages" aria-label="{t('Related pages','相关页面')}"><a href="{base}paper/">{t('Paper record','论文著录')}</a><a href="{base}research-context/">{t('Research context','技术定位')}</a><a href="{base}concepts/teach-and-grow-learning/">{t('Teach-and-Grow Learning','Teach-and-Grow Learning')}</a><a href="{base}concepts/training-free-robot-learning/">{t('Training-free robot learning','免训练机器人学习')}</a><a href="{base}concepts/skill-block/">{t('Skill Block','Skill Block')}</a><a href="{base}concepts/skill-library/">{t('Skill Library','Skill Library')}</a><a href="{base}concepts/experience-memory/">{t('Experience Memory','Experience Memory')}</a><a href="{base}concepts/retraining-tax/">{t('Retraining tax','再训练成本')}</a><a href="{base}research/agentic-robotics-2026/">{t('Agentic robotics in 2026','2026 年智能体机器人')}</a><a href="{base}concepts/gpt-6-robotic-arm/">{t('GPT-6 robotic arm','GPT-6 机器人手臂')}</a><a href="{base}concepts/agent-as-policy/">{t('Agent as Policy','Agent as Policy')}</a><a href="{base}concepts/coding-agent-robotics/">{t('Coding agents for robots','面向机器人的 coding agent')}</a><a href="{base}concepts/physical-in-context-learning/">{t('Physical in-context learning','物理上下文学习')}</a><a href="{base}concepts/robot-agent-memory/">{t('Robot agent memory','机器人智能体记忆')}</a><a href="{base}concepts/">{t('All concepts','全部概念')}</a><a href="{base}glossary/">{t('Glossary','术语表')}</a><a href="{base}faq/">{t('FAQ','问答')}</a></nav></footer><dialog id="image-dialog" aria-label="{t('Enlarged research image','放大的研究图片')}"><button class="dialog-close" aria-label="{t('Close image','关闭图片')}">×</button><img alt=""><p></p></dialog>
</body></html>'''

EN_HTML = display_html(page('en'))
(site/'index.html').write_text(EN_HTML)
(site/'zh').mkdir()
ZH_HTML = display_html(page('zh'))
...
SUBPAGES = ([PAGES.PAPER, PAGES.RESEARCH_CONTEXT, PAGES_HOT.HUB]
            + PAGES.CONCEPTS + PAGES_EXTRA.CONCEPTS + PAGES_HOT.CONCEPTS)
SUB_URLS = []
for _pg in SUBPAGES:
    for _lang in ("en", "zh"):
